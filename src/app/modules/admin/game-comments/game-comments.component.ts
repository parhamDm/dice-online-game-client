...
export class GameCommentsComponent implements OnInit {
  ...
  elements: any = [];
  previous: any = [];
  headElements = ['نام بازی', 'کاربر امتیاز دهنده','امتیاز','کامنت','تاریخ کامنت','عملیات'];
  ...
  resetTable(){
   ...
   getCommsNotAll(){
     ...
     this.userService.getGameUnapprovedCommentsList().subscribe(model=>{
   ...
   getCommsAll(){
    ...
    this.userService.getGameCommentsList().subscribe(model=>{
      this.elements= model.result;
  ...
  approveOrdeny(id:number,operation:number){ //1 approve //2 declined
    this.userService.changeStatusGame(id,operation).subscribe(model=>{
      this.getCommsNotAll();
    });
